...
export const updateUserPlan = async (userId: string, plan: SubscriptionPlan): Promise<void> => {
  try {
    ...
    await updateDoc(userRef, {
      ...
      'subscription.expiresAt': Timestamp.fromDate(new Date(now.getTime() + planConfig.days * 24 * 60 * 60 * 1000)),
      'subscription.lastUpdated': Timestamp.fromDate(now)
    });
  } catch (error) {
...
export const saveScript = async (userId: string, script: string, analysis: string): Promise<string> => {
  try {
